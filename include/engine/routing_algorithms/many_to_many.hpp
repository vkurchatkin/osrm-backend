    ...
    : public BasicRoutingInterface<DataFacadeT, ManyToManyRouting<DataFacadeT>>
{
  ...
  public:
                            ...
                            std::vector<EdgeWeight> &result_table) const
    {
        ...
        if (StallAtNode<true>(node, source_weight, query_heap))
        {
            return;
        }
        RelaxOutgoingEdges<true>(node, source_weight, query_heap);
    }
                             ...
                             SearchSpaceWithBuckets &search_space_with_buckets) const
    {
        ...
        if (StallAtNode<false>(node, target_weight, query_heap))
        {
            return;
        }

        RelaxOutgoingEdges<false>(node, target_weight, query_heap);
    }

    template <bool forward_direction>
    inline void
    RelaxOutgoingEdges(const NodeID node, const EdgeWeight weight, QueryHeap &query_heap) const
    {
    ...
    template <bool forward_direction>
    inline bool
    StallAtNode(const NodeID node, const EdgeWeight weight, QueryHeap &query_heap) const
    {
        for (auto edge : super::facade->GetAdjacentEdgeRange(node))
        {
            const auto &data = super::facade->GetEdgeData(edge);
